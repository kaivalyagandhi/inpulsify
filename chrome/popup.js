...
chrome.runtime.onMessage.addListener( function(request, sender, sendResponse) {
    if(request.action == 'read') {
        $.post('http://app-inpulsify.rhcloud.com/keywords', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
            chrome.tabs.query({active:true, currentWindow:true}, function(tabs) {
                chrome.tabs.sendMessage(tabs[0].id, {action: "readKeywords", keywords: resp.keywords});
            });
        });
        $.post('http://app-inpulsify.rhcloud.com/sentiment', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
            img_src = null;
        ...
        $.post('http://app-inpulsify.rhcloud.com/language', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
            $('#language').text(resp.language[0]);
        });
        $.post('http://app-inpulsify.rhcloud.com/virality', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
            $('#virality').prepend("<h2>Virality Score</h2>\n");
            $('#viralityval').prepend((resp.virality[0]*100).toString().substring(0,4) + "%");
        });
        $.post('http://app-inpulsify.rhcloud.com/political', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
            var pieData = [
            {
        ...
        $.post('http://app-inpulsify.rhcloud.com/text_tags', {'data_to_analyze': request.data}, function(resp) {
            resp = JSON.parse(resp);
